...
import CareerStatsMarquee from "./CareerStatsMarquee";
...
export default function Career() {
    ...
    const getX = (year: number) => {
        ...
        return (
            padding.left +
            ((year - CAREER_YEAR_RANGE.start) / range) * innerWidth
        );
    };
    ...
    const getY = (level: number) => {
        return (
            graphHeight -
            padding.bottom -
            (level / CAREER_LEVEL_RANGE.max) * innerHeight
        );
    };
    ...
    const gridLevels = Array.from(
        { length: CAREER_LEVEL_RANGE.max + 1 },
        (_, i) => i,
    );
                    ...
                    <div className="career-sec_history">
                        {CAREER_DATA.map((item, i) => (
                            ...
                            >
                                ...
                                <h4 className="career-sec_title">
                                    {item.title}
                                </h4>
                                <p className="career-sec_desc">
                                    {item.description}
                                </p>
                            </div>
                        ))}
