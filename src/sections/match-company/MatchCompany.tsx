...
function MatchCompany({ onResult, onMatch }: MatchCompanyProps) {
    ...
    const handleIntroSubmit = useCallback((e: React.FormEvent) => {
        e.preventDefault();
    ...
    return (
        <section
            className="match-company-sec"
            aria-label="우리 회사가 맞을까요?"
        >
            ...
            <div className="_cont">
                {phase === "intro" && (
                    <div ref={introRef} className="_intro" role="status">
                        <p className="_intro-text">
                            {textWithLineBreaks(INTRO_TEXT)}
                        </p>
                        <form
                            className="_intro-form"
                        ...
                        >
                            ...
                            <button
                                type="submit"
                                className="_btn _btn--yes _intro-submit"
                            >
                                시작하기
                            </button>
                        </form>
                ...
                {phase === "question" && currentQuestion && (
                    <>
                        <div className="_progress" aria-live="polite">
                            {questionIndex + 1} /{" "}
                            {MATCH_COMPANY_QUESTIONS.length}
                        </div>
                    ...
                    <>
                        <div ref={resultRef} className="_result">
                            <p className="_result-text">
                                {textWithLineBreaks(
                                    isMatch
                                        ? "귀사와 제가 좋은 시너지를 낼 거라는 확신이 들었습니다.\n준비된 제 역량을 구체적으로 검토해 주시면 감사하겠습니다."
                                        : NO_MATCH_MESSAGE,
                                )}
                            </p>
                        </div>
